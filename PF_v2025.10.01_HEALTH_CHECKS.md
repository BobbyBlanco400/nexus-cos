...
|---------|----------|------|-------------------|
| Core API Gateway | `https://n3xuscos.online/api/health` | 4000 | HTTP 200 + JSON status |
| Gateway Health | `https://n3xuscos.online/health/gateway` | - | HTTP 200 |

### PUABO NEXUS Services (Box Truck & Fleet Ecosystem)

| Service | Endpoint | Port | Expected Response |
|---------|----------|------|-------------------|
| AI Dispatch | `https://n3xuscos.online/puabo-nexus/dispatch/health` | 3231 | HTTP 200 + JSON status |
| Driver App Backend | `https://n3xuscos.online/puabo-nexus/driver/health` | 3232 | HTTP 200 + JSON status |
| Fleet Manager | `https://n3xuscos.online/puabo-nexus/fleet/health` | 3233 | HTTP 200 + JSON status |
| Route Optimizer | `https://n3xuscos.online/puabo-nexus/routes/health` | 3234 | HTTP 200 + JSON status |

### V-Suite Services

| Service | Endpoint | Port | Expected Response |
|---------|----------|------|-------------------|
| VScreen Hollywood | `https://n3xuscos.online/v-suite/screen/health` | 8088 | HTTP 200 + JSON status |
| V-Prompter Pro | `https://n3xuscos.online/v-suite/prompter/health` | 3011 | HTTP 200 + JSON status |

### Media & Entertainment

| Service | Endpoint | Port | Expected Response |
|---------|----------|------|-------------------|
| Nexus Studio AI | `https://n3xuscos.online/nexus-studio/health` | 3006 | HTTP 200 + JSON status |
| Club Saditty | `https://n3xuscos.online/club-saditty/health` | 3007 | HTTP 200 + JSON status |
| PUABO DSP | `https://n3xuscos.online/puabo-dsp/health` | 3003 | HTTP 200 + JSON status |
| PUABO BLAC | `https://n3xuscos.online/puabo-blac/health` | 3005 | HTTP 200 + JSON status |

### Authentication & Payment

| Service | Endpoint | Port | Expected Response |
|---------|----------|------|-------------------|
| Nexus ID OAuth | `https://n3xuscos.online/auth/health` | 3001 | HTTP 200 + JSON status |
| Nexus Pay Gateway | `https://n3xuscos.online/payment/health` | 3008 | HTTP 200 + JSON status |
