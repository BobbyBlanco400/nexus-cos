...
configure_nginx() {
    ...
    # Use safe deployment function that handles backup, validation, and rollback
    safe_deploy_nginx_heredoc "/etc/nginx/sites-available/nexuscos" "true" << 'NGINX_EOF'
...
NGINX_EOF
    ...
    # Reload Nginx (already validated by safe_deploy_nginx_heredoc)
    reload_nginx || {
